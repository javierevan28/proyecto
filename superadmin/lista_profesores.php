...
if (isset($_GET['ajax']) && $_GET['ajax'] == 1) {
    ...
    if ($accion === 'activar' || $accion === 'desactivar') {
        ...
        $resultado = $profesorModel->toggleActivo($id, $nuevoEstado);
        
        if (isset($resultado['success'])) {
            echo json_encode([
                'success' => true, 
                'message' => $accion === 'activar' ? '✅ Profesor activado correctamente' : '✅ Profesor desactivado correctamente',
                'activo' => $nuevoEstado
            ]);
        } else {
            echo json_encode([
                'success' => false, 
                'message' => $resultado['error'] ?? 'Error al actualizar el profesor'
            ]);
        }
        exit;
    }
    
    // Obtener datos de un profesor para editar
    if ($accion === 'get') {
        if ($id <= 0) {
            echo json_encode(['success' => false, 'message' => 'ID de profesor inválido']);
            exit;
...
$tiposList = ['titular' => 'Titular', 'frances' => 'Francés', 'cocurricular' => 'Cocurricular'];
...
<style>
    ...
    .btn-modal-confirm.confirm-activate {
        background: #059669;
    }

    .btn-modal-confirm.confirm-activate:hover {
        background: #047857;
    }
...
<main class="container">
    ...
    <div class="card">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
            ...
            <a href="alta_profesor.php" class="btn btn--accent">➕ Nuevo Profesor</a>
        </div>
                    ...
                    <tbody>
                        <?php foreach ($profesores as $i => $p): ?>
                            ...
                            <tr>
                                ...
                                <td><span class="badge"><?= ucfirst($p['tipo'] ?? 'titular') ?></span></td>
                                <td>
                                    <?php if ($esActivo): ?>
                                ...
                                <td>
                                    <div class="table-actions">
                                        <button class="btn-action btn-edit" onclick="editarProfesor(<?= $p['id'] ?>)">
                                            Editar
                                        </button>
                                        <?php if ($esActivo): ?>
...
<div id="editModal" class="modal-overlay">
    <div class="modal-content">
        ...
        <h3 class="modal-title">✏️ Editar Profesor</h3>
        <form id="editForm">
            ...
            <div class="modal-grid">
                ...
                <div class="modal-field">
                    ...
                    <select id="editTipo" name="tipo" required>
                        <?php foreach ($tiposList as $val => $label): ?>
                            <option value="<?= $val ?>"><?= $label ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
...
<div class="modal-overlay" id="confirmModal">
    <div class="modal-confirm">
        ...
        <div class="modal-confirm__actions">
            <button class="btn-modal-confirm" id="confirmBtn">Confirmar</button>
...
function abrirModalConfirmacion(id, action, nombre) {
    ...
    if (action === 'activar') {
        title.textContent = '🔄 Activar Profesor';
        body.textContent = `¿Confirmas activar a "${nombre}"?`;
        btn.className = 'btn-modal-confirm confirm-activate';
        btn.textContent = 'Activar';
    } else {
        title.textContent = '⛔ Desactivar Profesor';
...
function editarProfesor(id) {
    ...
    let boton = null;
    botones.forEach(b => {
        if (b.closest('tr').querySelector(`[onclick*="editarProfesor(${id})"]`)) {
            boton = b;
        }
    });
    ...
    fetch(`?ajax=1&accion=get&id=${id}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('editTipo').value = p.tipo || 'titular';
                document.getElementById('editFechaNacimiento').value = p.fecha_nacimiento;
        ...
        .catch(error => {
            console.error('Error:', error);
            showToast('Error al cargar los datos del profesor', 'error');
        })
        .finally(() => {
            if (boton) {
