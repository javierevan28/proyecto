...
if ($materiaId) {
    $stmtM = $db->prepare("SELECT es_ausencias, es_disciplina FROM materias WHERE id = ? LIMIT 1");
    $stmtM->bind_param('i', $materiaId);
    $stmtM->execute();
    $rowM = $stmtM->get_result()->fetch_assoc();
    if ($rowM) {
        $esAusencias  = (int)$rowM['es_ausencias']  === 1;
        $esDisciplina = (int)$rowM['es_disciplina'] === 1;
    }
}
...
// ── POST: borrar calificaciones (normal / inglés / artes — todas en calificaciones) ──
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['borrar_todas'])) {
    ...
    $stmt->execute() ? $mensaje = "✅ Se eliminaron calificaciones correctamente."
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_ausencias'])) {
    ...
    foreach ($_POST['dias'] as $alumnoId => $d) {
        ...
        $stmt->bind_param('iiii', $alumnoId, $cicloId, $periodoPost, $d);
        if (!$stmt->execute()) $errores++;
    }
    $mensaje = $errores ? "⚠️ Hubo $errores error(es) al guardar ausencias." : "✅ Ausencias guardadas correctamente.";
}

// ── POST: guardar disciplina ──────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_disciplina'])) {
    $periodoPost = (int)$_POST['periodo'];
    $errores     = 0;
    $stmt = $db->prepare("
        INSERT INTO calificaciones_disciplina (alumno_id, aspecto_id, periodo, calificacion, capturado_por)
        VALUES (?, ?, ?, ?, 1)
    ...
    foreach ($_POST['calificacion'] as $alumnoId => $aspectos) {
        foreach ($aspectos as $aspectoId => $cal) {
            ...
            $stmt->bind_param('iiid', $alumnoId, $aspectoId, $periodoPost, $cal);
            if (!$stmt->execute()) $errores++;
        }
    }
    $mensaje = $errores ? "⚠️ Hubo $errores error(es) al guardar disciplina." : "✅ Disciplina guardada correctamente.";
}

// ── POST: guardar normales / inglés / artes — misma tabla calificaciones ──
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar'])) {
    ...
    foreach ($_POST['calificacion'] as $alumnoId => $aspectos) {
        foreach ($aspectos as $aspectoId => $cal) {
            $cal = $cal === '' ? null : (float)$cal;
            if ($cal !== null && ($cal < 0 || $cal > 10)) continue;
            $stmt->bind_param('iiiid', $alumnoId, $asignacionId, $aspectoId, $periodoPost, $cal);
            $stmt->execute();
        }
    }
...
if ($materiaId && $seccion && $grado && $grupo) {
    ...
    $stmtM2 = $db->prepare("SELECT nombre FROM materias WHERE id = ?");
    $stmtM2->bind_param('i', $materiaId);
    $stmtM2->execute();
    $nombreMateria = ($r = $stmtM2->get_result()->fetch_assoc()) ? $r['nombre'] : '';
    ...
    if ($esAusencias) {
        ...
        $stmt = $db->prepare("
            ...
            LEFT JOIN ausencias au
                   ON au.alumno_id = al.id AND au.ciclo_id = ? AND au.periodo = ?
        ...
        // Aspectos: primero por asignacion_id, si vacío por grado/seccion global
        $stmtAsp = $db->prepare("
            SELECT id, nombre, porcentaje, orden
            FROM asignacion_disciplina_aspectos
            WHERE asignacion_id = ? AND activo = 1 ORDER BY orden
        ");
        ...
        if (empty($aspectos)) {
            $stmtAsp2 = $db->prepare("
                SELECT id, nombre, porcentaje, orden
                FROM asignacion_disciplina_aspectos
                WHERE asignacion_id IS NULL AND seccion = ? AND grado = ? AND activo = 1 ORDER BY orden
            ");
            $stmtAsp2->bind_param('si', $seccion, $grado);
            $stmtAsp2->execute();
            $aspectos = $stmtAsp2->get_result()->fetch_all(MYSQLI_ASSOC);
        }

        $stmtAl = $db->prepare("
            SELECT id AS alumno_id, nombre, apellido_paterno, apellido_materno
            FROM alumnos WHERE seccion = ? AND grado = ? AND grupo = ? AND activo = 1
            ORDER BY apellido_paterno, apellido_materno, nombre
        ");
        $stmtAl->bind_param('sis', $seccion, $grado, $grupo);
        $stmtAl->execute();
        $alumnosRaw = $stmtAl->get_result()->fetch_all(MYSQLI_ASSOC);

        foreach ($alumnosRaw as &$al) {
            $al['califs'] = [];
            foreach ($aspectos as $asp) {
                $stmtC = $db->prepare("
                    SELECT calificacion FROM calificaciones_disciplina
                    WHERE alumno_id = ? AND aspecto_id = ? AND periodo = ?
                ");
                $stmtC->bind_param('iii', $al['alumno_id'], $asp['id'], $periodo);
                $stmtC->execute();
                $rc = $stmtC->get_result()->fetch_assoc();
                $al['califs'][$asp['id']] = $rc ? $rc['calificacion'] : '';
            }
        }
        unset($al);
        $alumnos = $alumnosRaw;
        ...
        if ($asignacionId) {
            ...
            $idsVistos = [];
            foreach ($alumnosRaw as $al) {
                if (in_array($al['alumno_id'], $idsVistos)) continue;
                $idsVistos[]  = $al['alumno_id'];
                $al['califs'] = [];
                $stmtC = $db->prepare("
                    SELECT aspecto_id, calificacion FROM calificaciones
                    WHERE alumno_id = ? AND asignacion_id = ? AND periodo = ?
                ");
                $stmtC->bind_param('iii', $al['alumno_id'], $asignacionId, $periodo);
                $stmtC->execute();
                $res2 = $stmtC->get_result();
...
.filtros-grid {
    ...
    gap: 0.75rem; margin-bottom: 0.75rem; align-items: end;
}
.filtros-idioma-materia {
    display: grid;
    grid-template-columns: 160px 1fr;
    gap: 0.75rem; margin-bottom: 1rem; align-items: end;
}
...
.idioma-btn { display: flex; gap: 0.5rem; margin-bottom: 0.3rem; }
.idioma-btn button {
    ...
    transition: all .15s; color: var(--color-muted);
}
.idioma-btn button.active { background: var(--color-primary); border-color: var(--color-primary); color: white; }
.header-acciones {
...
<main class="container">
    <div class="card">
            ...
            <div class="filtros-idioma-materia">
                <div class="form-group">
                    ...
                    <div class="idioma-btn">
                        <button type="button" class="<?= $idioma === 'espanol' ? 'active' : '' ?>" onclick="setIdioma('espanol')">🇲🇽 Español</button>
                        <button type="button" class="<?= $idioma === 'ingles'  ? 'active' : '' ?>" onclick="setIdioma('ingles')">🇺🇸 Inglés</button>
                    </div>
                </div>
                <div class="form-group">
                    <label>Materia</label>
                    <select name="materia_id" onchange="this.form.submit()">
                        ...
                        <?php foreach ($materiasActuales as $m): ?>
                            <option value="<?= $m['id'] ?>" <?= $materiaId === (int)$m['id'] ? 'selected' : '' ?>>
        ...
        <?php if ($materiaId && $alumnos): ?>
                ...
                <table class="tabla-calif">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><input type="number" name="dias[<?= $al['alumno_id'] ?>]" value="<?= (int)$al['dias'] ?>" min="0" max="31" class="aus-input"></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <?php elseif ($esDisciplina): ?>
            <!-- ── DISCIPLINA ── -->
            <div class="header-acciones">
                <h3 style="margin:0;">🎓 <?= htmlspecialchars($nombreMateria) ?> — Periodo <?= $periodo ?></h3>
            </div>
            <?php if (empty($aspectos)): ?>
                <p class="empty-state">⚠️ No hay aspectos configurados para disciplina.</p>
            <?php else: ?>
            <form method="POST">
                <input type="hidden" name="periodo" value="<?= $periodo ?>">
                ...
                <table class="tabla-calif">
                    ...
                    <tbody>
                        <?php $i = 1; foreach ($alumnos as $al):
                            ...
                            foreach ($aspectos as $asp) {
                                $v = $al['califs'][$asp['id']] ?? '';
                                if ($v !== '' && $v !== null && (float)$asp['porcentaje'] > 0) {
                                    $suma += (float)$v * ((float)$asp['porcentaje'] / 100);
                                    $peso += (float)$asp['porcentaje'];
                                }
                            }
                            $prom = $peso > 0 ? round($suma) : '—';
                        ?>
                        <tr>
                            <td><?= $i++ ?></td>
                            <td class="alumno-nombre"><?= htmlspecialchars($al['apellido_paterno'] . ' ' . ($al['apellido_materno'] ?? '') . ', ' . $al['nombre']) ?></td>
                            <?php foreach ($aspectos as $asp): ?>
                                <td><input type="number" name="calificacion[<?= $al['alumno_id'] ?>][<?= $asp['id'] ?>]" value="<?= $al['califs'][$asp['id']] ?? '' ?>" min="0" max="10" step="0.1" class="cal-input"></td>
                            <?php endforeach; ?>
                ...
                <button type="submit" name="guardar_disciplina" class="btn" style="margin-top:1rem;">💾 Guardar disciplina</button>
            </form>
            <?php endif; ?>

            <?php else: ?>
            <!-- ── NORMAL / INGLÉS / ARTES — una sola rama ── -->
            <div class="header-acciones">
                <h3 style="margin:0;">📖 <?= htmlspecialchars($nombreMateria) ?> — Periodo <?= $periodo ?></h3>
                <button type="button" class="btn btn-danger" id="btnBorrarTodo">🗑️ Borrar todas las calificaciones</button>
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <?php foreach ($aspectos as $asp):
                                ...
                                if ($val !== '') {
                                    $suma += (float)$val * ((float)$asp['porcentaje'] / 100);
                                    $peso += (float)$asp['porcentaje'];
                                }
                            ?>
                                <td><input type="number" name="calificacion[<?= $al['alumno_id'] ?>][<?= $asp['id'] ?>]" value="<?= $val ?>" min="0" max="10" step="0.1" class="cal-input"></td>
                            <?php endforeach;
        ...
        <?php elseif ($materiaId && !$esAusencias && !$esDisciplina && !$asignacionId): ?>
...
function mostrarNotificacion(mensaje, tipo) {
    const e = document.querySelector('.notificacion-flotante');
    if (e) e.remove();
    const n = document.createElement('div');
    n.className = 'notificacion-flotante ' + tipo;
    n.innerHTML = mensaje;
