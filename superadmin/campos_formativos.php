<?php
// superadmin/campos_formativos.php
...
$db      = getConexion();
$modelo  = new CampoFormativoModel($db);
...
if ($accion === 'desactivar' && $editId > 0) {
    ...
    header('Location: campos_formativos.php?msg=' . $msg . '&detalle=' . urlencode($resultado['error'] ?? ''));
    exit;
}
...
$campos    = $modelo->listarTodos();

$pageTitle = 'Superadmin › Campos Formativos';
...
<main class="container">
  ...
  <?php if ($msgRedir === 'activado'): ?>
    <p class="alert alert--success" role="status">✅ Campo formativo activado.</p>
  <?php elseif ($msgRedir === 'desactivado'): ?>
    <p class="alert alert--success" role="status">✅ Campo formativo desactivado.</p>
  ...
  <div style="display:grid; grid-template-columns:1fr 1.6fr; gap:1.5rem; align-items:start;">
    ...
    <section class="card">
      <h2 class="section-title">
        <?= $editando ? '✏️ Editar campo formativo' : '➕ Nuevo campo formativo' ?>
      </h2>
      ...
      <form method="POST" novalidate>
        ...
        <div class="form-group">
          ...
          <input
            ...
            placeholder="ej. LENGUAJES" required maxlength="100"
          >
        </div>
        ...
        <div style="display:flex; gap:.6rem; flex-wrap:wrap;">
          <button class="btn" type="submit">
            <?= $editando ? 'Guardar cambios' : 'Crear campo formativo' ?>
          </button>
          <?php if ($editando): ?>
            <a class="btn btn--muted" href="campos_formativos.php">Cancelar</a>
          <?php endif; ?>
        </div>
      </form>
    </section>
    ...
    <!-- ── Listado ───────────────────────────────────────────── -->
    <section>
      <h2 class="section-title">Campos formativos (<?= count($campos) ?>)</h2>

      <?php if (empty($campos)): ?>
        <p class="empty-state">Aún no hay campos formativos registrados.</p>
      <?php else: ?>
          ...
          <tbody>
            <?php foreach ($campos as $c): ?>
              <?php
                ...
                $urlActivar  = 'campos_formativos.php?accion=activar&id='    . $c['id'];
                $urlDesact   = 'campos_formativos.php?accion=desactivar&id=' . $c['id'];
                $urlEditar   = 'campos_formativos.php?accion=editar&id='     . $c['id'];
              ?>
              <tr>
                <td><?= $c['orden'] ?></td>
                  ...
                  <div class="table-actions">
                    ...
                    <?php if ($esActivo): ?>
                      <button
                        ...
                        type="button"
                        data-href="<?= $urlDesact ?>"
                        data-title="Desactivar campo formativo"
                    ...
                    <?php else: ?>
                      <button
                        ...
                        data-title="Activar campo formativo"
