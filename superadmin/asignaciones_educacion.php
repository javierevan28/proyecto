<?php
// superadmin/asignaciones_educacion.php
...
if ($accion === 'desactivar' && $editId > 0) {
    ...
    $stmt->execute();
    header('Location: asignaciones_educacion.php?msg=desactivado');
    exit;
}
...
if ($accion === 'activar' && $editId > 0) {
    ...
    $stmt->bind_param('i', $editId);
    $stmt->execute();
    header('Location: asignaciones_educacion.php?msg=activado');
    exit;
}
...
if ($accion === 'eliminar' && $editId > 0) {
    ...
    header('Location: asignaciones_educacion.php?msg=eliminado');
    exit;
}
...
// FILTRAR materias disponibles: solo Educación Física y Tecnología
$nombresEducacion = ['Educación Física', 'Tecnología'];
$todasMaterias = $materiaModelo->listarActivas();
$materiasForm = array_filter($todasMaterias, function($m) use ($nombresEducacion) {
    return in_array($m['nombre'], $nombresEducacion);
});
...
// Obtener TODAS las asignaciones y filtrar para mostrar solo Edu. Física y Tecnología
...
foreach ($todasAsignaciones as $key => $grupo) {
    ...
    foreach ($grupo as $asignacion) {
        if (in_array($asignacion['materia_nombre'], $nombresEducacion)) {
...
$pageTitle = 'Superadmin › Asignaciones - Educación Física y Tecnología';
...
<main class="container">
  ...
  <div style="display:grid; grid-template-columns:1fr 1.8fr; gap:1.5rem; align-items:start;">
    ...
    <section class="card">
      <h2 class="section-title">➕ Nueva asignación - Educación Física y Tecnología</h2>
    ...
    <section>
      ...
      <?php if (empty($asignaciones)): ?>
        <p class="empty-state">Aún no hay asignaciones de Educación Física y Tecnología para este ciclo.</p>
      <?php else: ?>
                    ...
                    <td style="text-align:center;">
                      <div class="table-actions">
                        <a href="javascript:void(0)" class="btn btn--sm <?= $esActivo ? 'btn--warning' : 'btn--success' ?> action-btn"
                           data-url="asignaciones_educacion.php?accion=<?= $esActivo ? 'desactivar' : 'activar' ?>&id=<?= $a['id'] ?>"
                        ...
                        <a href="javascript:void(0)" class="btn btn--sm btn--danger action-btn"
                           data-url="asignaciones_educacion.php?accion=eliminar&id=<?= $a['id'] ?>"
                           data-title="Eliminar"
