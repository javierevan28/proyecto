        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <select id="estatus" name="estatus" required>
              <option value="">Selecciona…</option>
              <option value="nuevo_ingreso" <?= (($_POST['estatus'] ?? '') === 'nuevo_ingreso') ? 'selected' : '' ?>>🆕 Nuevo Ingreso</option>
              <option value="reinscripcion" <?= (($_POST['estatus'] ?? '') === 'reinscripcion') ? 'selected' : '' ?>>🔄 Reinscripción</option>
              <option value="regular" <?= (($_POST['estatus'] ?? '') === 'regular') ? 'selected' : '' ?>>✅ Regular</option>
              <option value="baja" <?= (($_POST['estatus'] ?? '') === 'baja') ? 'selected' : '' ?>>❌ Baja</option>
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <select id="seccion" name="seccion" required>
              ...
              <?php foreach (['maternal', 'preescolar', 'primaria', 'secundaria'] as $sec): ?>
                <option value="<?= $sec ?>" <?= (($_POST['seccion'] ?? '') === $sec) ? 'selected' : '' ?>><?= ucfirst($sec) ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <select id="grado" name="grado" required>
              <option value="">Selecciona…</option>
              <?php for ($i = 1; $i <= 6; $i++): ?>
                <option value="<?= $i ?>" <?= (($_POST['grado'] ?? '') == $i) ? 'selected' : '' ?>><?= $i ?>°</option>
              <?php endfor; ?>
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <select id="grupo" name="grupo" required>
              ...
              <?php foreach (['A', 'B', 'C', 'D'] as $grp): ?>
                <option value="<?= $grp ?>" <?= (($_POST['grupo'] ?? '') === $grp) ? 'selected' : '' ?>><?= $grp ?></option>
              <?php endforeach; ?>
            </select>
          </div>
          ...
          <div class="form-group full">
            ...
            <select id="padre_id" name="padre_id" required>
              ...
              <?php foreach ($padres as $p): ?>
                <option value="<?= $p['id'] ?>" <?= (($_POST['padre_id'] ?? '') == $p['id']) ? 'selected' : '' ?>>
